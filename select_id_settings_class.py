...
class select_id_settings_class:
	...
	def select_mri(self,row_data_setting, list_mri, setting_is_protocol,high_diversity_protocols_selection):
		...
		if high_diversity_protocols_selection:
			divided_filtered_protocols_mri_list = self.divide_protocols(filtered_mri_list)
			shuffle(divided_filtered_protocols_mri_list)
		else:
			divided_filtered_protocols_mri_list=[]
			divided_filtered_protocols_mri_list.append(filtered_mri_list)
	...
	def generate_id_settings(self,high_diversity_protocols_selection):
					...
					for index_select in range(len(settings_decoded[index])):
						one_setting_first_row_selected.append(
							self.select_mri(settings_decoded[index][index_select][0], self.list_mri,
									   alphabet in self.settings_using_protocols,high_diversity_protocols_selection))
