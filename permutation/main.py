...
settings_using_protocols_only=['SS']# List of experimental setting names, selected by protocol name attribute for training, evaluating, and testing
high_diversity_protocols_selection = False # If set to True, maximum diversity protocols will be applied in MRI image selection. Otherwise, MRIs will be selected randomly.
#####################################################################################################################################
#####################################################################################################################################
def main(select_id_we_want,repeat_number,load_selected_ids,output_path,
	...
	if load_selected_ids.lower() =="none":
		...
		settings_selected_all=select_id_settings.generate_id_settings(high_diversity_protocols_selection)
	...
	# Permutation
	mypermutation = copy.deepcopy(settings_selected_all_paths)
	test_all = []
	validation_all = []
	train_all = []
	for item in mypermutation:
		for row_item in item:
			for mri_id in row_item[0]:
				train_all.append(mri_id)
			for mri_id in row_item[1]:
				validation_all.append(mri_id)
			for mri_id in row_item[2]:
				test_all.append(mri_id)
	random.shuffle(train_all)
	random.shuffle(validation_all)
	random.shuffle(test_all)
	label_train_all = [sublist[3] for sublist in train_all]
	label_validation_all = [sublist[3] for sublist in validation_all]
	label_test_all = [sublist[3] for sublist in test_all]

	random.shuffle(label_train_all)
	random.shuffle(label_validation_all)
	random.shuffle(label_test_all)

	for i in range(len(train_all)):
		train_all[i][3] = label_train_all[i]
	for i in range(len(validation_all)):
		validation_all[i][3] = label_validation_all[i]
	for i in range(len(test_all)):
		test_all[i][3] = label_test_all[i]

	for index_p in range(len(mypermutation)):
		for index_p_i in range(len(mypermutation[index_p])):
			mypermutation[index_p][index_p_i][0] = []
			for mri_id in train_all:
				mypermutation[index_p][index_p_i][0].append(mri_id)
			mypermutation[index_p][index_p_i][1] = []
			for mri_id in validation_all:
				mypermutation[index_p][index_p_i][1].append(mri_id)
			mypermutation[index_p][index_p_i][2] = []
			for mri_id in test_all:
				mypermutation[index_p][index_p_i][2].append(mri_id)
	settings_selected_all_paths = mypermutation
